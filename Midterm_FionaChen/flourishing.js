...
class FlowerStream {
  ...
  displayGlow() {
    ...
    if (random() < 0.55) {
      petals.push(
        new FlowerParticle(
          this.currentX + random(-this.w * 0.3, this.w * 0.3),
          random(-30, 0)
        )
      );
    }
...
class FlowerParticle {
  constructor(x, y) {
    ...
    this.life = 0;
    this.maxLife = int(random(180, 320));   
    this.fadeStart = this.maxLife * 0.65;   
  }
    ...
    if (this.life > this.fadeStart) {
      this.alpha = map(this.life, this.fadeStart, this.maxLife, this.baseAlpha, 0);
    }
  ...
  getColor() {
    ...
    offscreen() {
    return this.y > height + 40 || this.life >= this.maxLife || this.alpha <= 0;
  }
}
